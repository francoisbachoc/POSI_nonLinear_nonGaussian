...
noise_gen = function(n,sigma) {
  runif(n=n, min = -sqrt(3)*sigma, max = sqrt(3)*sigma)
}
name="holm_highDim_lar_correlated_uniform"
...
for (i in 1:nmc) {
  ...
  for (k in 1:nStep) {
    ...
    #TG inference
    larinf = larInf(lar,alpha=1-alpha,k=k,type='active',gridrange = c(-vgridrange[k],vgridrange[k]))
  ...
  M = Msteps[1:nStep]
  larinf = larInf(lar,alpha=1-alpha,k=nStep,type='all',gridrange = c(-vgridrange[nStep],vgridrange[nStep]))
  for (k in 1:nStep) {
    #specifying the target
    itarget = M[k]
    target = target_holm(X,M,theta,itarget)
