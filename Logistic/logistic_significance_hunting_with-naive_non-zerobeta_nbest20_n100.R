...
##############################
set.seed(1)
name="logistic_significance_hunting_with-naive_non-zerobeta_nbest20_n100"
...
for (i in 1:nmc) {
  ...
  #Naive Inference
  ICNaive = IC_logistic_naive(X,M,Y,itarget,alpha,maxL1)
...
save(file=save_name,n,p,alpha,nbest,beta_0,rho,lambda,vCoveragePOSI,vLengthPOSI,vCoverageNaive,vLengthNaive,mM,vitarget)
